...
def searchForVehicles(args):
    ...
    try:
        ...
        while not done:
            ...
            useLocalData = True
            # debug end
            df = vehicles.update_vehicles(useLocalData = useLocalData)
            if df is not None:
                #replace nan with None in the DataFrame to ease computations later on 
                df = df.replace({np.nan: None})
                # debug begin
                if debugState == 1:
                    # change entry to be new vin1 so it shows up as added
                    df.at[3, "VIN"] = "NewVIN1"
                    df.at[3, "Model"] = "XSE"
                    df.at[3, "Pre-Sold"] = False
                    df.at[3, "Hold Status"] = "Available"
                    df.at[3, "Markup"] = 55
                    df.at[3, "Selling Price"] = 0
                    df.at[3, "Dealer State"] = "IA"
                    df.at[3, "Options"] = " Premium Package Weather Package " 
                    # change entry to be new vin2 so it shows up as added
                    df.at[10, "VIN"] = "NewVIN2"
                    df.at[10, "Model"] = "XSE"
                    df.at[10, "Pre-Sold"] = False
                    df.at[10, "Hold Status"] = "Available"
                    df.at[10, "Markup"] = 66
                    df.at[10, "Selling Price"] = 0
                    df.at[10, "Dealer State"] = "IL"
                    df.at[10, "Options"] = " Premium Package Weather Package " 
                    #change entry so it passes through filter and shows as added to last matches list
                    df.at[14, "Model"] = "XSE"
                    df.at[14, "Pre-Sold"] = False
                    df.at[14, "Hold Status"] = "Available"
                    df.at[14, "Markup"] = 101
                    df.at[14, "Selling Price"] = 45000
                    df.at[14, "Dealer State"] = "IL"
                    df.at[14, "Options"] = " Premium Package Weather Package Silly" 
                    #change entry so it passes through filter and shows as added to last matches list
                    df.at[25, "Model"] = "XSE"
                    df.at[25, "Pre-Sold"] = False
                    df.at[25, "Hold Status"] = "Available"
                    df.at[25, "Markup"] = 301
                    df.at[25, "Selling Price"] = 44000
                    df.at[25, "Dealer State"] = "MO"
                    df.at[25, "Options"] = " Premium Package Weather Package Silly2" 
                elif debugState == 2:
                    #change entry so it shows up as modified.
                    df.at[14, "Model"] = "XSE"
                    df.at[14, "Pre-Sold"] = False
                    df.at[14, "Hold Status"] = "Available"
                    df.at[14, "Markup"] = 601
                    df.at[14, "Selling Price"] = 43123
                    df.at[14, "Dealer State"] = "MN"
                    df.at[14, "Options"] = " Premium Package Weather Package Silly3" 
                    #change entry so it shows up as modified
                    df.at[25, "Model"] = "XSE"
                    df.at[25, "Pre-Sold"] = False
                    df.at[25, "Hold Status"] = "Available"
                    df.at[25, "Markup"] = 701
                    df.at[25, "Selling Price"] = 44457
                    df.at[25, "Dealer State"] = "NE"
                    df.at[25, "Options"] = " Premium Package Weather Package Silly4" 
                elif debugState == 3:
                    pass
                elif debugState == 4:
                    pass
